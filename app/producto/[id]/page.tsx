...
import { ArrowLeft, ShoppingCart, MapPin, Clock, Phone, Mail, Banknote, CreditCard, Link2, Truck, Minus, Plus, Package, MessageCircle, Copy, Check, Share2, Building2, Store } from 'lucide-react';
...
export default function ProductoDetailPage() {
  ...
  useEffect(() => {
    if (params?.id) {
      ...
      // Extract the ID from the last segment after the last hyphen
      const slugParts = (params.id as string).split('-');
      ...
      <main className="flex-1 max-w-6xl mx-auto px-4 py-6 pb-20 w-full">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          >
            ...
            <p className="text-gray-600 text-lg whitespace-pre-line">{producto.description}</p>
            ...
            {/* Emprendedor info */}
            <div className={`rounded-xl p-6 shadow-sm space-y-4 ${producto.emprendedor?.tipo === 'EMPRESA' ? 'bg-blue-50 border-2 border-blue-200' : 'bg-white'}`}>
              {/* Header con logo para empresas */}
              <div className="flex items-center gap-4">
                {producto.emprendedor?.tipo === 'EMPRESA' && producto.emprendedor?.logoUrl ? (
                  <div className="w-16 h-16 rounded-xl overflow-hidden bg-white border border-blue-200 flex-shrink-0 shadow-sm">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img 
                      src={producto.emprendedor.logoUrl} 
                      alt={producto.emprendedor.businessName}
                      className="w-full h-full object-contain"
                    />
                  </div>
                ) : producto.emprendedor?.tipo === 'EMPRESA' ? (
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <h2 className={`font-semibold text-lg ${producto.emprendedor?.tipo === 'EMPRESA' ? 'text-blue-800' : 'text-gray-800'}`}>
                      {producto.emprendedor?.businessName}
                    </h2>
                    {producto.emprendedor?.tipo === 'EMPRESA' && (
                  ...
                  {producto.emprendedor?.description && (
                    <p className="text-gray-600 text-sm mt-1">{producto.emprendedor.description}</p>
                  )}
                </div>
              </div>
              ...
              {/* Ver más de esta empresa button */}
              <Link
                href={`/comercio/${producto.emprendedor?.id}`}
              ...
              >
                ...
                Ver más de {producto.emprendedor?.tipo === 'EMPRESA' ? 'esta empresa' : 'este emprendedor'}
              </Link>
            </div>
